...
const deployContract = async(contractJson: any,  wallet: Wallet, ...args: any): Promise<Contract> => {
  const factory = new ContractFactory(contractJson.abi, contractJson.bytecode, wallet)
  let contract = await factory.deploy(...args)
  console.log(`Address: [${contract.address}], Tx: [${contract.deployTransaction.hash}]`)
...
const deployContracts = async (wallet: Wallet): Promise<void> => {
  ...
  if (!evaluatorContractAddress) {
    ...
    const transitionEvaluator = await deployContract(UnipigTransitionEvaluator, wallet)
    evaluatorContractAddress = transitionEvaluator.address
    console.log('UnipigTransitionEvaluator deployed!\n\n')
  } else {
    console.log(`Using UnipigTransitionEvaluator contract at ${evaluatorContractAddress}\n`)
  }

  let merkleUtilsCnontractAddress = process.env.DEPLOY_MERKLE_UTILS_CONTRACT_ADDRESS
  if (!merkleUtilsCnontractAddress) {
    ...
    console.log(`Using RollupMerkleUtils contract at ${merkleUtilsCnontractAddress}\n`)
  }


  console.log('Deploying RollupChain...')
  const rollupChain = await deployContract(RollupChain, wallet, evaluatorContractAddress, merkleUtilsCnontractAddress)
...
const deploy = async (): Promise<void> => {
  ...
  if (!network || network === 'local') {
    provider = new ethers.providers.JsonRpcProvider(process.env.DEPLOY_LOCAL_URL || 'http://127.0.0.1:7545')
  } else {
