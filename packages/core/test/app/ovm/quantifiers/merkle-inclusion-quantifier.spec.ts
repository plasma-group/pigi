...
class MockedMerkleTree implements MerkleTree {
  ...
  public async getLeaf(leafKey: BigNumber, rootHash?: Buffer): Promise<Buffer> {
    ...
    return leafKey.toString() in this.leaves ? this.leaves[leafKey.toString()] : undefined
  }
...
describe('MerkleTreeQuantifier', () => {
  describe('getAllQuantified', () => {
    ...
    it('returns undefined on empty tree', async () => {
      ...
      assert(result.results.length === 1, 'There should only be one result that is undefined.')
      assert(result.results[0] === undefined, 'There should only be one result that is undefined.')
      assert(!result.allResultsQuantified, 'All results should not be quantified if we are unable to find a leaf node in the tree.')
    })
    ...
    it('returns value on populated tree tree with a root that matches', async () => {
      ...
      assert(result.results.length === 1, 'There should only be one result that is the value.')
      assert(result.results[0].equals(value) , `Result value [${result.results[0].toString()}] found when expecting [${value.toString()}]`)
      assert(result.allResultsQuantified, 'All results should be quantified if we are able to find the leaf node in the tree.')
    })
